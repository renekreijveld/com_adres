...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_adres/assets/css/adres.css');
$document->addStyleSheet(JUri::root() . 'media/com_adres/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_adres');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_adres&task=adressen.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'adresList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_adres&view=adressen'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="adresList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_adres');
					$canEdit    = $user->authorise('core.edit', 'com_adres');
					$canCheckin = $user->authorise('core.manage', 'com_adres');
					$canChange  = $user->authorise('core.edit.state', 'com_adres');
					?>
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
						<td class="center">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'adressen.', $canChange, 'cb'); ?>
						</td>
						<?php endif; ?>
						<td>
							<?php echo $item->id; ?>
						...
						<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
							<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'adressen.', $canCheckin); ?>
						<?php endif; ?>
						<?php if ($canEdit) : ?>
							<a href="<?php echo JRoute::_('index.php?option=com_adres&task=adres.edit&id='.(int) $item->id); ?>">
